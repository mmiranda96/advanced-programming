...
void * set(Hash *h, void *key, void *value) {
    unsigned pos = h->hash(key, h->size);
    for (int i = 0; i < h->data[pos].count; i++) {
        if (h->compareKeys(key, h->data[pos].elements[i].key)) {
            void *prevValue = h->copyValue(h->data[pos].elements[i].value);
            h->data[pos].elements[i].value = h->copyValue(value);
            return prevValue;
        }
    }
    return NULL;
